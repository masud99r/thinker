...
exp_name = algo+"_"+env_name+"_"+str(start_level)+"_"+str(num_levels)+"_seed_"+str(seed)+"_maxtimestep_"+str(max_timestep)
...
def convert_obs_rad(obs, aug='crop'):
    ...
    del rand_crop
    return obs
def convert_obs_rad_cutout(obs):
    rand_cutout = Cutout_Color(batch_size=obs.shape[0])
    obs = obs.cpu().detach().numpy()
    obs = rand_cutout.do_augmentation(obs)
    obs = torch.as_tensor(obs, dtype=torch.float32).to(device)
    del rand_cutout
    return obs
...
def agent_loss(policy, model, dist_class, train_batch):
    ...
    elif algo == 'rad_cutout': # algo is a data augmentation
        obs = convert_obs_rad_cutout(train_batch[SampleBatch.CUR_OBS])
        train_batch[SampleBatch.CUR_OBS] = obs #.to(device)
        loss = ppo_surrogate_loss(policy, model, dist_class, train_batch) # average over original obs loss?
    else: # ppo
